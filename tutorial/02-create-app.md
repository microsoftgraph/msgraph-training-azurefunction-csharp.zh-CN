...
- 使用客户端凭据授予流身份验证，实现用于订阅和取消订阅用户收件箱通知的 API。 [](https://docs.microsoft.com/azure/active-directory/develop/v2-oauth2-client-creds-grant-flow)
- 实现 Webhook[以接收来自](https://docs.microsoft.com/graph/webhooks)Microsoft Graph更改通知，以及使用客户端凭据授予流访问数据。
...
1. 打开 **local.settings.js，** 将以下内容添加到 文件以允许来自 的测试应用程序的 URL 中的 `http://localhost:8080` CORS。
...
1. 打开浏览器并浏览到输出中显示的函数 URL，验证函数是否正常工作。 您应该在浏览器中看到以下消息 `Welcome to Azure Functions!` ：。

## <a name="create-single-page-application"></a>创建单页应用程序

1. 在要创建项目的目录中打开 CLI。 创建一个名为 **TestClient 的** 目录以保存 HTML 和 JavaScript 文件。

1. 在 **TestClient** **index.htm创建** 一个名为index.html 的新文件，并添加以下代码。

    :::code language="html" source="../demo/TestClient/index.html" id="indexSnippet":::

    这将定义应用的基本布局，包括导航栏。 它还添加了以下内容：

    - [Bootstrap](https://getbootstrap.com/) 及其支持的 JavaScript
    - [Font提供](https://fontawesome.com/)
    - [适用于 JavaScript 2.0 (MSAL.js) Microsoft 身份验证库](https://github.com/AzureAD/microsoft-authentication-library-for-js/tree/dev/lib/msal-browser)

    > [!TIP]
    > 此页面包括一个 favicon、 (`<link rel="shortcut icon" href="g-raph.png">`) 。 可以删除此行，也可以从g-raph.png **下载** GitHub。 [](https://github.com/microsoftgraph/g-raph)

1. 在 **TestClient** 目录中创建一个名为 **style.css** 的新文件并添加以下代码。

    :::code language="css" source="../demo/TestClient/style.css":::

1. 在 **TestClient** **ui.js** 一个名为ui.js文件并添加以下代码。
...
> [!NOTE]
> 本节包含有关使用 [dotnet-serve 在](https://github.com/natemcmaster/dotnet-serve) 开发计算机上运行简单测试 HTTP 服务器的说明。 不需要使用此特定工具。 可以使用您喜欢的任何测试服务器来为 **TestClient** 目录提供服务。
...
1. 将 CLI 中的当前目录更改为 **TestClient** 目录并运行以下命令以启动 HTTP 服务器。

    ```Shell
    dotnet serve -h "Cache-Control: no-cache, no-store, must-revalidate" -p 8080
    ```

1. 打开浏览器，并导航到 `http://localhost:8080`。 页面应呈现，但当前没有任何按钮可正常工作。

## <a name="add-nuget-packages"></a>添加 Nuget 程序包

在继续之前，请安装一些NuGet包，你稍后会使用它。

- [Microsoft.Azure.Functions.Extensions，](https://www.nuget.org/packages/Microsoft.Azure.Functions.Extensions) 用于启用 Azure Functions 项目中的依赖关系注入。
- [Microsoft.Extensions.Configuration。UserSecrets，](https://www.nuget.org/packages/Microsoft.Extensions.Configuration.UserSecrets) 用于读取 .NET 开发密码存储 [中的应用程序配置](https://docs.microsoft.com/aspnet/core/security/app-secrets)。
